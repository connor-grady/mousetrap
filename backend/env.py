...
Every read of ``os.environ`` lives here. Directory overrides are exposed raw
(as provided, or ``None`` when unset); ``backend.paths`` owns the repo root and
applies the filesystem defaults. Non-path settings are returned final.
"""
