...
VIP_POINTS_COST: dict[int, int] = {4: 5_000, 8: 10_000}  # weeks -> points; 90/max is variable
WEDGE_POINTS_COST = 50_000
UPLOAD_POINTS_PER_GB = 500

# MaM only accepts these upload-credit purchase amounts (GB); minimum 50 as of January 2026.
VALID_UPLOAD_CREDIT_GB = (50, 100)

WedgeMethod = Literal["points", "cheese"]


def parse_vip_weeks(raw: str | int) -> Literal["max"] | int:
    """Normalize a raw ``weeks`` value to ``"max"`` or an integer week count.

    ``"max"`` and ``"90"`` select the variable-cost max purchase; every other
    value must parse as an integer. Raises ``ValueError`` otherwise.
    """
    if str(raw).lower() in ("max", "90"):
        return "max"
    try:
        return int(raw)
    except (TypeError, ValueError):
        raise ValueError(f"Invalid VIP weeks value: {raw!r}") from None
...
async def buy_wedge(
    mam_id: str, method: WedgeMethod = "points", proxy_cfg: dict[str, Any] | None = None
) -> dict[str, Any]:
