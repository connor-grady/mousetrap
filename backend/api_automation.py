...
async def _min_points_guardrail(
    *,
    ...
    min_points = cfg.get("perk_automation", {}).get("min_points")
...
async def manual_vip(request: Request) -> dict[str, Any]:
    ...
    label_desc = "Max me out!" if is_max else f"{weeks} weeks"
    spec = _PerkSpec(
        purchase_type="vip",
        log_tag="ManualVIP",
        noun="VIP",
        amount="max" if is_max else weeks,
        detail_summary=label_desc,
        log_descriptor="VIP (max)" if is_max else f"VIP ({weeks} weeks)",
        status_success=f"Purchased VIP ({label_desc})",
        status_failed=f"VIP purchase failed ({label_desc})",
        notify_details={"weeks": "max" if is_max else weeks},
        event_details={},
    )
    # Max/90-week VIP has variable cost; the guardrail only applies to known fixed durations
    if not is_max:
        purchase_cost = _VIP_POINTS_COST.get(int(weeks))
